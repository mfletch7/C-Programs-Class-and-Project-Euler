//Problem 8 from project Euler, given a 1000 digit number, find the product of 13 adjacent numbers that is the greatest
//Created by Michael Fletcher, April 26, 2016

#include <iostream>     //cout, cin
#include <string>       //String class for using string type variables
#include <sstream>      //osstringstream conversion
#include <iomanip>      //Setprecision
using namespace std;

int main()
{
    string bigNum = "";
    int num1[1000] = {0};
    int temp1 = 0;
    string singNum = "";
    double product = 1.0;
    double highProduct = 0.0;

    for(int i = 0; i <= 999; i += 1)
    {
        //While this could be reduced by two lines, I have found that a straight variable works better for conversions than attempting direct using arrays
        singNum = bigNum.at(i);
        istringstream convert(singNum);
        convert >> temp1;
        num1[i] = temp1;
    }
    for (int i = 0; i <= (999-12); i += 1)
    {
        product = 1.0;
        for (int n = i; n < (i+13); n += 1)
        {
            product *= num1[n];
            if (n > 995)
                cout << "n is equal to " << n << endl;
        }
        if(product > highProduct)
            highProduct = product;
    }
    cout << "The highest product of 13 numbers is " << fixed << setprecision(0) << highProduct << endl;
    return 0;
}
